    ...
    public sealed partial record LoginInfo
    {
        ...
        private static string? ValidateLogin(string? login)
        {
            ...
            if (!RegexLoginCheck().IsMatch(login))
                throw new ValidationException("Логин содержит недопустимые символы");
        ...
        private static string? ValidateEmail(string? email)
        {
            ...
            if (email.Length > 100)
                throw new ValidationException($"Email не может быть длиннее 50 символов");
        ...
        private static string? ValidatePhone(string? value)
        {
            ...
            if (value.Length != 11 || !(value.StartsWith('7') || value.StartsWith('8')))
                throw new ValidationException("Номер телефона должен быть в формате 7XXXXXXXXXX");
        ...
        private static string GenerateLogin(string? email, string? phone)
        {
            string basePart = email?.Split('@')[0]?.Replace(".", "_")
                         ...
                         ?? throw new ArgumentNullException(email, "Почему-то один из параметров пустой. Email или телефон не должны были быть тут пустыми.");
