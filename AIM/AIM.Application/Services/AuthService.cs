...
namespace AIM.Application.Services
{
    ...
    public class AuthService(IUserRepository userRepository, IRefreshTokenRepository refreshTokenRepository, IConfiguration configuration) : IAuthService
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(request.Role)) throw new Exception("Пользователь должен иметь роль");

            Roles role = (Roles)Enum.Parse(typeof(Roles), request.Role);
            if (role == Roles.ADMIN || role == Roles.NONE) throw new Exception("Данную роль невозможно зарегестрировать");

            User newUser = new(Guid.NewGuid(), request.LoginInfo, PasswordHasherService.HashPassword(request.Password), request.FIO, request.Messangers, role);
        ...
        public async Task<LoginResponseDto> Authenticate(LoginRequestDto request)
        {
            ...
            User userAccount = await userRepository.GetByLoginAsync(request.UserName) ?? throw new Exception("Неверный логин или пароль.");
            ...
            if (userAccount.Priveleges == Roles.BANNED)
                throw new Exception("Вы были заблокированны от использованияя сервиса.");
        ...
        public async Task<LoginResponseDto> RefreshToken(RefreshTokenRequest request)
        {
            ...
            RefreshToken refreshToken = await refreshTokenRepository.GetByTokenAsync(Guid.Parse(request.RefreshToken)) ?? throw new Exception("Токен не действителен");
            ...
            return user is null ? throw new Exception("Пользователь был удалён") : await GenerateJwtToken(user);
        }
